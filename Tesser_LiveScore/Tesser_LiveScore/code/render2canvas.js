...
function writeStaffLines()
{
			...
			for (var staves in stafflines[measures]) {
				for (var lines in stafflines[measures][staves]){
					...
					{
					for (var d = 0; d < dest.length; d++) {
						path += "M" + stafflines[measures][staves][lines][0] + " " + dest[d] + " L" + stafflines[measures][staves][lines][2] + " " + dest[d] + " ";	
						if (lineCount > 500) {
...
{		
		...
		//renderedMessages.name = 1334 + "-renderedMessages";
	...
	if (measurerange.length > 0) setMeasureRange(measurerange[0], measurerange[1], measurerange[2], measurerange[3]);
}
    ...
    switch (messagename) {
       ...
       case "printNoteText":
        ...
        case "printComposer":
            ...
            break;
        case "RenderMessage":
			if (msg[msg.length - 3] == "rendered") {
			renderedMessages.set(rm++, msg);
			var e = new Dict();
			e.parse(msg[msg.length - 1]);
			if (e.contains("picster-element")) {
 			var picster = e.get("picster-element");
 			//if (picster.contains("name")) picster.remove("name");
			//if (picster.contains("bounds")) picster.remove("bounds");
...
function nTET(steps, system)
{
				...
				return system[Math.round(factor1 * steps / 12 * direction + 6 / steps)];
}
...
function pagenumber()
{
	//post("pagenumber", paperSize, scoreLayout, "\n");
