...
for(var event in anchors){
	...
	var userBeans = json[key]["userBean"];
	if 	(Array.isArray(userBeans)) {
		for (var i = 0; i < userBeans.length; i++) {
			if (userBeans[i]["Message"].indexOf("hold") == -1) outlet(0, "addRenderedMessageToSelectedNotes", parseFloat(userBeans[i]["Xoffset"]), parseFloat(userBeans[i]["Yoffset"]), userBeans[i]["Message"]);
		}
	}
	else if (userBeans["Message"].indexOf("hold") == -1) outlet(0, "addRenderedMessageToSelectedNotes", parseFloat(userBeans["Xoffset"]), parseFloat(userBeans["Yoffset"]), userBeans["Message"]);
	}
	...
	outlet(0, "setNoteVisible", "false");
	outlet(0, "clearSelection");
	if (!selection) outlet(0, "setScoreSize", (scoreSize * factor) + playheadPosition + scoreRightMargin, parseFloat(scoreAttributes["score"]["HEIGHT"]));
...
{
	outlet(0, "addRenderedMessageToSelectedNotes", 4, 0, "rendered hold[0] " + d);
}
